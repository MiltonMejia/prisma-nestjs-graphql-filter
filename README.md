...
-   Node.js `>=20.19`.
-   This package is **ESM-only** (`"type": "module"`), so your project must be ESM too.
-   Peer dependencies: `@nestjs/common`, `@nestjs/core`, `@nestjs/graphql`, `graphql` and `reflect-metadata`.
...
> Prisma 7 requires an explicit `output` in the generator, and the generated client is ESM. Import the `Prisma` namespace from your generated client path as shown above.
...
## Getting Prisma fields

Use `GetFields` to feed a decorator with every key of a Prisma enum object. This works with the enums generated by Prisma 7:

```typescript
import { GetFields, WhereInput } from 'prisma-nestjs-graphql-filter';
import { Prisma } from './generated/prisma/client.js';

@WhereInput({ type: String, fields: GetFields(Prisma.HelloWorldScalarFieldEnum) })
class ExampleInput {}
```

`GetIdFields` does the same but keeps only `id` and relation id fields (keys ending in `_id`).
